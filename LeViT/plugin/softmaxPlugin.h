...
namespace nvinfer1
{
static const char *PLUGIN_NAME {"softmax"};
static const char *PLUGIN_VERSION {"1"};

class softmaxPlugin : public IPluginV2DynamicExt
{
private:
    std::string name_;
...
public:
    softmaxPlugin(const std::string &name, float epsilon):
        name_(name), epsilon_(epsilon)
    {
        WHERE_AM_I();
    }

    softmaxPlugin(const std::string &name, const void *data, size_t length):
        name_(name)
    {
        WHERE_AM_I();
    ...
    {
        WHERE_AM_I();
        return new softmaxPlugin(name_, epsilon_);
    }
...
public:
    ...
    IPluginV2 *createPlugin(const char *name, const PluginFieldCollection *fc) noexcept override
    {
        ...
        return new softmaxPlugin(name, epsilon);
    }

    IPluginV2 *deserializePlugin(const char *name, const void *serialData, size_t serialLength) noexcept override
    {
        WHERE_AM_I();
        return new softmaxPlugin(name, serialData, serialLength);
    }
